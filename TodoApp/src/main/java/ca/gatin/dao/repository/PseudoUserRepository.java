...
public interface PseudoUserRepository extends JpaRepository<PseudoUser, Long> {
	...
	@Query(value = "SELECT * FROM user WHERE LOWER(activationkey) = LOWER(:activationkey) LIMIT 1", nativeQuery = true)
	...
	@Query(value = "SELECT COUNT(*) FROM user WHERE LOWER(activationkey) = LOWER(:activationkey)", nativeQuery = true)
